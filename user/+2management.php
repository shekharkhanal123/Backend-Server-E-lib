...
if(isset($_POST['reserve']))
{
  ...
  if(isset($_SESSION['Email']))
  {
      ...
      {  
        ...
        // {
          $query="SELECT * FROM management WHERE  Id IN($column) ";
          $ans=mysqli_query($conn,$query);
          if($ans == true)
          {
            $_SESSION['faculty']="Management";
            foreach($ans as $row)
            {   
...
<div class=content>
    <div class="title">+2 Management</div>
    <div class="course">
    <div>
    <div class="container">
    <div class="bitem">
    ...
    <thead align="center">
      <tr class="thead">
        <th style="padding: 5px 6px;">class<input style="width: 40px; margin-left: 10px;" type="number" placeholder="-lvl-" name="class"/> </th>
    ...
    <?php 
      ...
      { 
        $class=$_POST['class'];
        if(!empty($class)){
        $query="SELECT * FROM management WHERE Class='$class'";
        $ans=mysqli_query($conn,$query);
        include("../Query/Get_Data_From_Class.php");
        }
        elseif(empty($class)){
